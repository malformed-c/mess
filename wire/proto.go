// Package wire is the mess protocol: the types a client and the daemon
// exchange, plus enough client to talk to a running daemon.
//
// It exists so a UI can be its own module. mess itself is deliberately
// dependency-free, and a build tag cannot preserve that — `go mod tidy`
// considers every build configuration, so a tagged-out import is still a
// requirement. Only a separate module keeps the core clean, and a separate
// module needs something importable to talk to, which package main is not.
package wire
